...
public class Word2VecManager {
     ...
     * @param query
     */
    public String getMatchingQuery(String lang, String query) throws UnknownWordException {
        if (containsQuery(lang, query)) {
            return query;
        } else if (containsDegradedQuery(lang, query)) {
            return Lexicon.degrade(query);
        } else {
            throw new UnknownWordException(query);
        }
    }
     ...
     * @param query
     */
    public List<Double> getRawVector(String lang, String query) throws UnknownWordException, UnsupportedLanguageException {
        if (!supports(lang)) {
            throw new UnsupportedLanguageException(lang);
        }
        return models.get(lang).getRawVector(query).asList();
    }
    ...
    /**
     * Returns a list of up to maxCount matching similar words for the
     * given query and language. The query is degraded if needed (see
     * {@link Lexicon.degrade}).
     *
     * @param lang
     * @param query
     * @param maxCount
    ...
    public List<Match> getMatches(String lang, String query, int maxCount) throws UnsupportedLanguageException, MalformedQueryException, UnknownWordException {
        ...
        return models.get(lang).getMatches(getMatchingQuery(lang, query), maxCount);
    }
